"""
evaluate_ae.py — 학습된 AE로 val 200장을 복원하고 baseline과 동일 지표로 평가.

[알고리즘 1] 학습 patch 생성 시 손상 덩어리 단위 crop 적용 (precompute_patches.py)
...
PATCH_SIZE = 256
STRIDE = 64  # precompute_patches.py와 통일
...
def main(ckpt_path, n_images=200, save_viz_n=5, seed=0, merge_mode="average"):
    ...
    for i, row in val_df.iterrows():
        ...
        restored, coverage = restore_with_ae(model, img, mask2d, device, merge_mode=merge_mode)
        ...
        if i < save_viz_n:
            ...
            out_path = os.path.join(VERIFY_DIR, f"ae_{fname}_m0.png")
    ...
    print(f"\n[완료] AE 평가 결과 {len(res_df)}건 저장 → {RESULTS_PATH}")
