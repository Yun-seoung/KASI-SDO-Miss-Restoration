"""
evaluate_lama.py — LaMa Generator로 MISS1 val 200장을 복원하고 baseline과 동일 지표로 평가.
...
[core_crop] AE(evaluate_ae.py)에서 검증된 방식을 LaMa에 동일하게 이식.
            가중평균으로 인한 밴드(띠) 아티팩트 제거 목적.
...
[seam correction 미적용] AE 실험에서 덧셈형/affine형 둘 다 지표·시각 품질
            개선 효과가 없어 제외하기로 결정한 바 있어, LaMa에도 동일하게
            seam correction 없이 순정 core_crop만 적용.
[버그 방지] 최종 합성 시 core/패치 사각형 전체가 아니라 실제 손상 픽셀
            (mask_full>0)에만 덮어쓰도록 처리 — 그렇지 않으면 멀쩡한 원본
            픽셀까지 모델 출력으로 덮어써서 블록 모양의 큰 아티팩트가 생김.
[fallback 지표] fallback_mask를 반환받아 실제 손상 픽셀(mask_full>0)과
            교집합한 개수만 세도록 처리 — 손상과 무관한 패치 margin 영역까지
            다 세어져서 지표가 실제 위험도를 과대평가하는 것을 방지.
...
def core_crop_merge(h, w, tile_results, patch_size, core_margin):
    ...
    tile_results: [(rs, cs, pred_denorm, mask_patch), ...]
    """
...
def restore_with_lama(G, img_full, mask_full, device, index, seed, merge_mode="average",
                       patch_size=PATCH_SIZE, stride=None, noise_std=EVAL_NOISE_STD):
    """
    [알고리즘 1] 손상 덩어리 단위로 crop 위치 선정 (average/best_tile).
                 core_crop은 이미지 전체 기준 전역 grid 사용.
    ...
    # 블록 모양의 큰 아티팩트가 생김. (average/best_tile도 동일하게 안전 적용)
...
def main(ckpt_path, n_images=200, save_viz_n=5, seed=0, merge_mode="average"):
    ...
    for i, row in val_df.iterrows():
        ...
        restored, coverage, fallback_px = restore_with_lama(
            G, img, mask2d, device, index=i, seed=seed, merge_mode=merge_mode)
        ...
        if i < save_viz_n:
            ...
            out_path = os.path.join(VERIFY_DIR, f"lama_{fname}_m0_{merge_mode}.png")
    ...
    print(f"\n[완료] LaMa(MISS1) 평가 결과 {len(res_df)}건 저장 → {RESULTS_PATH}")
