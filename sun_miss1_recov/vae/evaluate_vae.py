"""
evaluate_vae.py — 학습된 VAE로 MISS1 val 200장을 복원하고 baseline과 동일 지표로 평가.
...
RESULTS_PATH = os.path.join(ROOT_DIR, "results", "vae_results.csv")
...
def restore_with_vae(model, img_full, mask_full, device, merge_mode="average",
    ...
    with torch.no_grad():
        for rs, cs in candidates:
            ...
            pred_n, mu, logvar = model(input_t)
...
def visualize(orig, damaged, restored, mask2d, out_path, vmax):
    ...
    titles_top = ["Original (full, downsampled)", "Damaged (full, downsampled)",
                  "VAE Restored (full, downsampled)", "Abs error (full, downsampled)"]
    ...
    titles_bot = ["Original (zoom)", "Damaged (zoom)", "VAE Restored (zoom)", "Abs error (zoom)"]
...
def main(ckpt_path, n_images=200, save_viz_n=5, seed=0, merge_mode="average"):
    ...
    print(f"[체크포인트 로드] {ckpt_path} (epoch {ckpt['epoch']+1}, beta={ckpt.get('beta', '?')})")
    ...
    for i, row in val_df.iterrows():
        ...
        restored, coverage, fallback_px = restore_with_vae(model, img, mask2d, device, merge_mode=merge_mode)
        ...
        if i < save_viz_n:
            ...
            out_path = os.path.join(VERIFY_DIR, f"vae_{fname}_m0_{merge_mode}.png")
    ...
    print(f"\n[완료] VAE(MISS1) 평가 결과 {len(res_df)}건 저장 → {RESULTS_PATH}")
